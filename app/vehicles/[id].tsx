    ...
    <View style={s.mItem}>
      ...
      <View style={s.mInfo}>
        <View style={s.mNameRow}>
          ...
          <CurrencyText amount={log.cost} size="sm" className="text-content-primary font-bold" />
        </View>
        <Text variant="paragraphS" tabular style={s.mMeta}>
          {new Date(log.date).toLocaleDateString(undefined, { dateStyle: "medium" })}
          {log.odometer ? ` · ${log.odometer} ${distanceUnit}` : ""}
        </Text>
        {log.notes ? <Text variant="paragraphS" style={s.mNotes}>{log.notes}</Text> : null}
      </View>
      ...
      <TouchableOpacity
        ...
        accessibilityLabel="Delete maintenance log"
...
export default function VehicleDetailScreen() {
  ...
  // Header element (stable Fragment, recreated but reconciled in place) so the
  // TextInputs inside keep focus while typing as the FlatList re-renders.
  const listHeader = (
    <>
      {/* Vehicle Stats Row */}
      ...
      <View style={s.card}>
        ...
        {/* Name */}
        <View style={s.row}>
          <Text variant="labelXs" style={s.rowLabel}>Name</Text>
          {isEditing ? (
            <TextInput
              value={name}
            ...
            <Text variant="labelL">{vehicle.name}</Text>
          )}
        </View>
        ...
        {isEditing && (
          <View style={s.row}>
            <Text variant="labelXs" style={s.rowLabel}>Type</Text>
            <View style={s.chips}>
              {VEHICLE_TYPES.map((t) => {
        ...
        <View style={s.rowInline}>
          <View style={s.col}>
            ...
            {isEditing ? (
              ...
              <Text variant="labelL">{vehicle.make || "—"}</Text>
            )}
          </View>
          <View style={s.col}>
            ...
            {isEditing ? (
              ...
              <Text variant="labelL">{vehicle.model || "—"}</Text>
            )}
          </View>
        </View>
        ...
        <View style={s.rowInline}>
          <View style={s.col}>
            ...
            {isEditing ? (
              ...
              <Text variant="labelL">{vehicle.year || "—"}</Text>
            )}
          </View>
          <View style={s.col}>
        ...
        {isEditing && (
          <View style={s.switchContainer}>
            <Text variant="labelM">Set as Active Vehicle</Text>
            <Switch
              value={isActive}
          ...
          <View style={{ gap: 8, marginTop: 8 }}>
            ...
            >
              {isSavingVehicle ? (
                ...
                <Text variant="labelM" style={{ color: accentColorContrast }}>Save Changes</Text>
              )}
            </TouchableOpacity>
            <TouchableOpacity
              ...
              style={s.dangerBtn}
            >
              <Text variant="labelM" style={s.dangerBtnText}>Delete Vehicle</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
      ...
      <View style={s.card}>
        <View style={s.cardHeaderRow}>
          ...
          >
            <Text variant="labelXs" style={showAddTaxProfile ? { color: DS.dangerText } : { color: accentColorContrast }}>
        ...
        {showAddTaxProfile && (
          <View style={{ marginTop: 14, borderTopWidth: 0.5, borderColor: DS.sep, paddingTop: 14 }}>
            ...
            {/* Year Input */}
            <View style={s.row}>
              <Text variant="labelXs" style={s.rowLabel}>Tax Year</Text>
              <TextInput
                value={taxYear}
              ...
              <Text variant="paragraphS" style={{ color: mileageEligibility.eligible ? accentColor : COLORS.destructive }}>
                ...
                {mileageEligibility.eligible ? (
                  <>
                    {" — "}
                    <CurrencyText
                      amount={mileageEligibility.ratePrimary}
            ...
            {/* Method Selection */}
            <View style={s.row}>
              <Text variant="labelXs" style={s.rowLabel}>Deduction Method</Text>
              <View style={s.chips}>
                <TouchableOpacity
                  onPress={() => setDeductionMethod("standard_mileage")}
                  accessibilityRole="button"
                  accessibilityState={{ selected: deductionMethod === "standard_mileage" }}
                  style={[s.chip, deductionMethod === "standard_mileage" && { borderColor: accentColorMid, backgroundColor: accentColorDim }]}
                >
                  <Text variant="labelM" style={[s.chipText, deductionMethod === "standard_mileage" && { color: accentColor }]}>
                    Standard Mileage
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  ...
                  accessibilityRole="button"
                  accessibilityState={{ selected: deductionMethod === "actual_expenses" }}
                  style={[s.chip, deductionMethod === "actual_expenses" && { borderColor: accentColorMid, backgroundColor: accentColorDim }]}
                >
                  <Text variant="labelM" style={[s.chipText, deductionMethod === "actual_expenses" && { color: accentColor }]}>
                    Actual Expenses
                  </Text>
                </TouchableOpacity>
              </View>
            ...
            {deductionMethod === "standard_mileage" && (
              <View style={{ gap: 12, marginBottom: 12 }}>
                <View style={s.rowInline}>
                  <View style={s.col}>
                    <Text variant="labelXs" style={s.rowLabel}>Primary Rate</Text>
                    <TextInput
                      value={ratePrimary}
                  ...
                  <View style={s.col}>
                    <Text variant="labelXs" style={s.rowLabel}>Secondary Rate (Opt)</Text>
                    <TextInput
                      value={rateSecondary}
                ...
                <View style={s.row}>
                  <Text variant="labelXs" style={s.rowLabel}>Rate Threshold (Opt distance)</Text>
                  <TextInput
            ...
            >
              <Text variant="labelM" style={{ color: accentColorContrast }}>Add Profile</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {taxProfiles.length === 0 ? (
          <View style={{ paddingVertical: 16, alignItems: "center", justifyContent: "center" }}>
            <Text variant="paragraphS" style={{ color: DS.textSecondary, textAlign: "center", marginTop: 8 }}>
              ...
              >
                <View style={{ flex: 1, gap: 4 }}>
                  ...
                  {p.deductionMethod === "standard_mileage" && (
                    <Text variant="paragraphS" tabular style={{ color: DS.textSecondary }}>
                ...
                <TouchableOpacity
                  onPress={() => handleDeleteTaxProfile(p.id)}
                  accessibilityRole="button"
                  accessibilityLabel={`Delete ${p.taxYear} tax profile`}
      ...
      <View style={{ gap: 12, marginBottom: 12 }}>
        <View style={s.cardHeaderRow}>
          ...
          >
            <Text variant="labelXs" style={showAddMaintenance ? { color: DS.dangerText } : { color: accentColorContrast }}>
        ...
        {showAddMaintenance && (
          <View style={s.card}>
            ...
            <View style={s.rowInline}>
              <View style={s.col}>
                <Text variant="labelXs" style={s.rowLabel}>Cost ($) *</Text>
                <TextInput
                  value={mCost}
              ...
              <View style={s.col}>
                <Text variant="labelXs" style={s.rowLabel}>Odometer ({profile.distanceUnit})</Text>
                <TextInput
                  value={mOdometer}
            ...
            <View style={s.row}>
              <Text variant="labelXs" style={s.rowLabel}>Notes</Text>
              <TextInput
                value={mNotes}
    ...
    <SafeAreaView style={s.safe} edges={["bottom", "left", "right"]}>
      ...
      <View style={[s.header, { paddingTop: insets.top + 10 }]}>
        <View style={s.headerLeft}>
          ...
          <View>
            <Text variant="headingM" numberOfLines={1}>{vehicle.name}</Text>
            <Text variant="paragraphS" style={s.headerSub} numberOfLines={1}>
        ...
        >
          <Text variant="labelM" style={isEditing ? { color: DS.dangerText } : { color: accentColorContrast }}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <FlatList
          ...
          ItemSeparatorComponent={MaintenanceLogSeparator}
...
const s = StyleSheet.create({
  ...
  dangerBtnText: { color: DS.dangerText },
  ...
  mMeta: { color: DS.textSecondary },
  mNotes: { color: DS.textSecondary, fontStyle: "italic", marginTop: 4 },
  deleteLogBtn: {
    width: 32,
    height: 32,
    borderRadius: 16,
