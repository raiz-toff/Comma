...
- **State:**
  ...
  - Verified compilation via `npx tsc --noEmit` successfully.
- [2026-06-29] Fixed GPS tracking not starting on shift begin: useGPSTracking.startTracking() was calling Expo Location.startLocationUpdatesAsync() with an unregistered TaskManager task (never defined), so the foreground service/notification never launched and no points were written. Rewired startTracking/stopTracking to call the native CommaTracker.startTracking()/stopTracking() module (same one endShift() reads from via temp_native_points).
- [2026-06-29] GPS hardening: useGPSTracking now also requests POST_NOTIFICATIONS at runtime before starting the native service, so the Android 13+ foreground-service 'Recording mileage' notification is visible. Verified full pipeline: native LocationTrackingService writes to filesDir/SQLite/comma.db (matches DATABASE_NAME) table temp_native_points (schema lat/lon/timestamp matches migration 0013); endShift reads + computes active/dead mileage; module autolinked via expo-module.config.json with play-services-location dep.
- [2026-06-29] Fixed LocationTrackingService notification not appearing: upgraded channel IMPORTANCE_LOW→DEFAULT and notification PRIORITY_LOW→DEFAULT so it appears in status bar. Added isLocationUpdateActive guard to prevent duplicate locationCallback registration on app restart (START_STICKY). Added onTaskRemoved override. Fixed onDestroy to reset isLocationUpdateActive flag.
- [2026-06-29] Fixed service dying on app swipe: added android:stopWithTask='false' to service manifest so Android keeps it alive when user removes app from recents. Added AlarmManager restart in onTaskRemoved as safety net for aggressive OEM battery managers (Samsung/MIUI).
- [2026-06-29] Fixed service dying when notification dismissed (Android 13+): added SharedPreferences tracking_active flag written by CommaTrackerModule startTracking/stopTracking. Service now checks flag in onStartCommand (stops itself if shift ended normally) and onDestroy (reschedules via AlarmManager if shift still active). Covers notification-dismissed, OEM kill, and START_STICKY restart cases.
