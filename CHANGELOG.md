...
- **Filter Home, Shifts and Analytics by vehicle (app)**: if you drive more than one vehicle, the same switcher that already filters by delivery platform now filters by vehicle too — all of them, a few, or just one. When you only have one platform and one vehicle there's nothing to pick, so the switcher collapses to a plain readout instead of an empty menu.
