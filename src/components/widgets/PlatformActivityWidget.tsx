  ...
  return (
    <View style={{ gap: 16, paddingTop: 4 }}>
      {topPlatforms.length === 0 ? (
        <Text style={{ fontSize: 13, color: "#9B9BA4", textAlign: "center", fontStyle: "italic", marginVertical: 10 }}>No shift data found</Text>
      ) : (
        topPlatforms.map((p) => {
          const platformDef = PLATFORMS[p.platform as PlatformKey];
          const color = platformDef?.color || "#F6F6F7";
          
          return (
            <View key={p.platform} style={{ gap: 8 }}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                <PlatformBadge platform={p.platform as PlatformKey} size="sm" />
                <Text style={{ fontSize: 15, fontWeight: "900", color: "#F6F6F7" }}>{p.share.toFixed(0)}%</Text>
              </View>
              <View style={{ height: 8, backgroundColor: "#1C1C21", borderRadius: 4, overflow: "hidden" }}>
