/**
 * Full-screen "set your backup password" flow.
 *
 * Deliberately a TAKEOVER, not a small dialog. Cloud Sync always encrypts, and this is the
 * only action in the app that can render the user's data PERMANENTLY unrecoverable if they
 * lose the result — there is no server, no reset link, no support path. Forgetting it means
 * resetting this device (see ForgotPasswordScreen) to keep syncing, which wipes everything on
 * it. A 6-line modal is the wrong weight for that; WhatsApp uses a full screen for the same
 * reason, and this used to be optional here too — now it's the mandatory first run of Sync.
 *
 * Two gates, in order:
...
export function SetBackupPasswordScreen({
  accentColor,
  accentColorContrast,
  onCancel,
  onConfirm,
  ...
  const submit = async () => {
    if (pw.length < MIN_BACKUP_PW) {
      setErr(`Use at least ${MIN_BACKUP_PW} characters.`);
      return;
    }
    if (pw !== confirm) {
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        <View style={s.hero}>
          ...
          <Text variant="paragraphM" style={{ textAlign: "center", maxWidth: 320 }}>
            Every sync is locked with a password only you know. Not even Google can read it.
          </Text>
        </View>
        ...
        <View style={[s.warnCard, { backgroundColor: withAlpha(C.destructive, 0.09), borderColor: withAlpha(C.destructive, 0.35) }]}>
          <View style={s.warnHead}>
            ...
            <Text variant="labelL" style={{ color: C.destructive }}>
              If you forget it, resetting is the only way forward
            </Text>
          </View>
          <Text variant="paragraphM" style={{ color: C.contentSecondary }}>
            ...
            path back to syncing is to reset this device, which wipes everything currently on it.
          </Text>
          <View style={s.warnBullets}>
            <WarnBullet text="We cannot email you a new one." />
            <WarnBullet text="Google cannot recover it." />
            <WarnBullet text="Resetting wipes this device's data to start over." />
          </View>
        ...
        >
          ...
          <Text variant="paragraphM" style={{ flex: 1, color: C.contentPrimary }}>
            I understand that if I forget this password, resetting this device — which wipes
            everything on it — is the only way to keep syncing.
          </Text>
        </Pressable>
      </ScrollView>
...
function CreateStage({
  ...
  const C = useColors();
  const s = useThemedStyles(makeStyles);
  const tooShort = pw.length > 0 && pw.length < MIN_BACKUP_PW;
  const ready = pw.length >= MIN_BACKUP_PW && confirm.length > 0 && !saving;
      ...
      >
        ...
        <View style={s.inputWrap}>
          ...
          {tooShort ? (
            <Text variant="paragraphS" style={{ color: C.warning, marginTop: 6 }}>
              {MIN_BACKUP_PW - pw.length} more character{MIN_BACKUP_PW - pw.length === 1 ? "" : "s"} needed
            </Text>
          ) : null}
        </View>
        ...
        <View style={[s.reminder, { borderColor: withAlpha(C.warning, 0.3), backgroundColor: withAlpha(C.warning, 0.07) }]}>
          ...
          <Text variant="paragraphS" style={{ flex: 1, color: C.contentSecondary }}>
            Comma has no copy of this password. If it's lost, resetting this device is the only
            way back to syncing — and it wipes everything currently on it.
          </Text>
        </View>
      </ScrollView>
